...
public class TrackerClient : MonoBehaviour {
    ...
    public bool PositionChange {
        get { return this.positionChange; }
    ...
    public Vector3 TeleportPosition {
        get { return this.teleportPosition; }
    ...
    void Update() {
        ...
        if (Input.GetMouseButtonDown(0)) // Mouse tap 
        {
        ...
        if(positionChange) {
            ...
            spineBase.position = new Vector3(teleportPosition.x, spineBase.position.y, teleportPosition.z);
    ...
    private void UpdateAvatarBody() {
        ...
        // Spine
        Vector3 spineUp = Utility.GetBoneDirection(spineShoulderJoint.Value, spineBaseJoint.Value);
        Vector3 spineRight = Utility.GetBoneDirection(rightShoulderJoint.Value, leftShoulderJoint.Value);
        ...
        // Left Arm
        leftShoulder.rotation = Utility.GetQuaternionFromRightUp(-Utility.GetBoneDirection(leftShoulderJoint.Value, spineShoulderJoint.Value), spineUp);
        leftElbow.rotation = Utility.GetQuaternionFromRightUp(-Utility.GetBoneDirection(leftWristJoint.Value, leftElbowJoint.Value), spineUp);
        leftArm.rotation = Utility.GetQuaternionFromRightUp(-Utility.GetBoneDirection(leftElbowJoint.Value, leftShoulderJoint.Value), spineUp);

        // Left Leg
        leftHip.rotation = Utility.GetQuaternionFromUpRight(-Utility.GetBoneDirection(leftKneeJoint.Value, leftHipJoint.Value), spineRight);
        leftKnee.rotation = Utility.GetQuaternionFromUpRight(-Utility.GetBoneDirection(leftAnkleJoint.Value, leftKneeJoint.Value), spineRight);

        // Right Arm
        rightShoulder.rotation = Utility.GetQuaternionFromRightUp(Utility.GetBoneDirection(rightShoulderJoint.Value, spineShoulderJoint.Value), spineUp);
        rightElbow.rotation = Utility.GetQuaternionFromRightUp(Utility.GetBoneDirection(rightWristJoint.Value, rightElbowJoint.Value), spineUp);
        rightArm.rotation = Utility.GetQuaternionFromRightUp(Utility.GetBoneDirection(rightElbowJoint.Value, rightShoulderJoint.Value), spineUp);

        // Right Leg
        rightHip.rotation = Utility.GetQuaternionFromUpRight(-Utility.GetBoneDirection(rightKneeJoint.Value, rightHipJoint.Value), spineRight);
        rightKnee.rotation = Utility.GetQuaternionFromUpRight(-Utility.GetBoneDirection(rightAnkleJoint.Value, rightKneeJoint.Value), spineRight);
    }
